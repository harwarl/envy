...
///
/// let key = "FOO";
/// let value = envparser::var(key).unwrap();
/// ```
...
/// use std::io;
///
/// let result: Vec<(String, String)> = envparser::vars().collect();
/// ```
pub fn vars() -> Vars {
    START.call_once(|| {
        dotenv().ok();
...
/// envparser::from_path(my_path.as_path());
/// ```
...
///
/// let my_path = env::home_dir().and_then(|a| Some(a.join("/.env"))).unwrap();
/// let iter = envparser::from_path_iter(my_path.as_path()).unwrap();
///
/// for item in iter {
...
/// ```
/// use envparser;
/// envparser::from_filename("custom.env").ok();
/// ```
///
...
/// ```
/// use envparser;
/// envparser::from_filename(".env").ok();
/// ```
...
/// ```no_run
/// use envparser;
/// let iter = envparser::from_filename_iter(".env").unwrap();
///
/// for item in iter {
///   let (key, val) = item.unwrap();
///   println!("{}={}", key, val);
...
/// This is usually what you wnat
/// It loads the .env file located in th environment's current directory or its params in sequence
///
/// # Examples
/// ```
/// use envparser;
...
/// use envparser;
///
/// for item in envparser::dotenv_iter().unwrap() {
/// let ( key, val ) = item.unwrap();
/// println!("{}={}", key, val);
/// }
/// ```
